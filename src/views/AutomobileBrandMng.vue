...
export default {
  ...
  mounted () {
    ...
    axios
      .get(
        'http://localhost:7300/mock/5ee3182c322a3759ecb9f07b/mei/brand'
      )
      .then(response => {
  ...
  methods: {
    ...
    filterBrand () {
      ...
      const filtersCountry = this.filters.country
      const filtersBrand = this.brandInfs.filter(item => {
    ...
    handleDelete (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        .then(() => {
          const brandInfs = this.brandInfs
          for (const index in brandInfs) {
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {
            ...
            .then(() => {
              ...
              this.$refs.formData.resetFields() // 重置表单，重置为初始值
              this.id = ''
              this.formVisible = false
            })
            .catch(() => {
