...
export default {
  ...
  mounted () {
    ...
    axios
      .get(
        'http://localhost:7300/mock/5ee3182c322a3759ecb9f07b/mei/stock'
      )
      .then(response => {
  ...
  methods: {
    ...
    filterStock () {
      ...
      const filterAutomobileStock = this.automobileStock.filter(item => {
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {
            ...
            .then(() => {
              ...
              const automobileStock = this.automobileStock
              for (const index in automobileStock) {
