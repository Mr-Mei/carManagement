...
export default {
  ...
  mounted () {
    ...
    axios
      .get(
        'http://localhost:7300/mock/5ee3182c322a3759ecb9f07b/mei/automobile'
      )
      .then(response => {
  ...
  methods: {
    ...
    filterAutomobile () {
      const filtersName = this.filters.name.trim()
      const filtersBrand = this.filters.brand.trim()
      const filtersCategory = this.filters.category
      const filtersAutomobile = this.automobileInfs.filter(item => {
    ...
    handleDelete (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        .then(() => {
          const automobileInfs = this.automobileInfs
          for (const index in automobileInfs) {
